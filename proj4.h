 ...
 * Added: threadArgs
 * 
 * Bundles multiple arguments for passing to pthread_create. Contains a pointer to the source grid, 
 * pointer to the result grid, the unsigned long sum being searched for, the first row index the thread 
 * responsible for, and the last row index the thread is respobsible for.
*/
typedef struct {
  grid *input;
  grid *output;
 ...
 * Added: threadWorker
 * 
 * Executed by every thread to process it's potion of the grid. Contains a pointer to a struct. Returns NULL
 * and modifies the output grid when matches are found.
*/
